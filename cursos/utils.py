...
def send_mail(tipo, vs, subject, email):
    ...
    mail = EmailMultiAlternatives(subject,
                                  ...
                                  'Cursos Mejorando.la <ventas@mejorando.la>',
                                  [email]
    )
...
def calculate(quantity, price):
    ...
    # aplicar el descuento solo cuando no sea cada 5
    # (que es cuando hay una plaza gratis mas)
    if (quantity % 5) != 0:
        discount += (price * 0.1) * (quantity - 1 if quantity < 5 else
                                     quantity - (5 * rate))
